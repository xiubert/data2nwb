...
def getPulsesFromLegend(legendMatPath: str) -> tuple:
    """
    Reads pulse metadata from a pulseLegend2P.mat file generated by tifPulseLegend2P.m.
    Fallback for getTifPulses when individual _Pulses.mat files are not available.

    xsg is stored as a plain basename string for stim entries, and a
    comma-separated string of basenames for map (multi-pulse) entries.

    Args:
        legendMatPath (str): path to pulseLegend2P.mat (saved as -v7.3)
    ...
    with h5py.File(legendMatPath, 'r') as h5:
        legend = h5['pulseLegend2P']

        tifs       = getH5stringList(legend['tif'][0], h5)
        ...
        out_names, out_sets, out_xsg = [], [], []

        for tif, stimDelay, ISI, name_ref, set_ref, xsg_ref in zip(
                tifs, stimDelays, ISIs,
    ...
    return out_tifs, out_types, out_delays, out_isis, out_names, out_sets, out_xsg


def getPulsesFromCSV(csvPath: str) -> tuple:
    """
    Reads pulse metadata from a pulseLegend2P CSV file.
    Fallback for getTifPulses / getPulsesFromLegend when no .mat files are available.

    Expected columns (see data/pulseLegend2P_example.csv):
